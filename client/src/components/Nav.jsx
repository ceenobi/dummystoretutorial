...
export default function Nav() {
  ...
  return (
    <div className="py-4 lg:py-12 px-4 sticky top-0 bg-slate-50 z-40">
      <div className="max-w-[1024px] mx-auto flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="relative">
            ...
            {cartQuantity > 0 && (
              <div className="badge absolute top-[-10px] right-[-10px] bg-success badge-sm text-white">
                {cartQuantity}
              </div>
            )}
          </div>
          {isAuthenticated ? (
            <>
              <details className="dropdown dropdown-end">
                <summary className="btn bg-transparent shadow-none border-0 m-1">
            ...
            <NavLink to="/login">
              <RxPerson className="cursor-pointer" size="24px" />
            </NavLink>
          )}
              ...
              <div className="drawer-side">
                ...
                <ul className="menu bg-base-200 text-base-content min-h-full w-80 py-4">
                  ...
                  {cats.map((item, index) => (
                    <li key={index}>
                      <NavLink
                        className="my-1 font-semibold capitalize"
                        to={`/products/${item.name}`}
                        onClick={() =>
                          (document.getElementById("my-drawer").checked = false)
                        }
